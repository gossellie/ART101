...
// jQuery event: log arrow clicks (non-destructive)
if (window.jQuery) {
    (function($){
        $(function(){
            $(document).on('click', '.control-group .arrow', function(){
                const feature = $(this).closest('.control-group').data('feature');
                const dir = $(this).data('direction');
                console.log('jQuery: arrow clicked', feature, dir);
            });
        });
    })(jQuery);
} else {
    console.log('jQuery not loaded - no jQuery events attached');
}
